...
Problem: Progress is currently stored locally on each device, preventing multi‑device learning and risking data loss.

Your Task: I would like to conduct the backend system for my current application. That system can control all the interaction between learners (via UI) an system. I also want to use database to manage the vocabulary system, tracking the activity, or support any functionality in the UI: next words, spaced repetition, etc...
...
Your Task: Refer to /construction/{unit name}/logical_design.md file for the logical design details. Generate a very simple and intuitive python implementation for the bounded context. Assume the repositories and the event stores are in-memory. Generate the classes in respective individual files but keep them in the /src/backend/{unit name} directory based on the proposed file structure. Ensure applying this backend feature to current application, let application live with this approach. Create a simple demo script that can be run locally to verify the implementation.
```
